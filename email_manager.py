...
class EmailManager:
    ...
    def render_email_template(self, newsletter_data: dict) -> str:
        """Render the newsletter using the beautiful email template."""
        try:
            template = self.template_env.get_template('email_newsletter_template.html')
            return template.render(
                newsletter=newsletter_data,
                website_url=os.getenv('WEBSITE_URL', 'https://your-newsletter-app.onrender.com')
            )
        except Exception as e:
            logger.error(f"Error rendering email template: {e}")
            # Fallback to beautiful HTML that matches web UI exactly
            return self.render_beautiful_email(newsletter_data)
    ...
    def send_newsletter(self, newsletter_data: dict, subject: str) -> Tuple[bool, str]:
        """Send newsletter to all active subscribers using beautiful template."""
        ...
        try:
            # Render the beautiful HTML email
            html_content = self.render_email_template(newsletter_data)
